var colors = [];
var class_map = d3.map()
var dataDir = "file://D:\\Workspace\\Datavisualize\\大作业\\data\\";
var bP = {};
var b = 30, bb = 150, height = 600, buffMargin = 1, minHeight = 14;
var bar_x = 50;
var c1 = [-200, 100], c2 = [-20, 250], c3 = [20, 300]; //Column positions of labels.
var data_map = d3.map()
!function () {
    for (var i = 0; i < right_cnt; i++) {
        colors[colors.length] = random_color()
    }
    ...
    bP.partData = function (data, p) {
        ...
        data.map(function (d, i) {
            class_map.set(d[0], d[3])
            class_map.set(d[1], d[4])
            if (data_map.has(d[0])) {
                var l = data_map.get(d[0])
                l[l.length] = d
                data_map.set(d[0], l)
            } else {
                data_map.set(d[0], [d])
    ...
    function edgePolygon(d) {
        return [0 + bar_x, d.y1, bb + bar_x, d.y2, bb + bar_x, d.y2 + d.h2, 0 + bar_x, d.y1 + d.h1].join(" ");
    }
                    ...
                    .on("click", function (d, i) {
                        ...
                        if (keep === "0" || keep === null) {
                            if (global_keep === "0") {
                                ...
                                // scatter(i)
                                showClassImg(svg, visData, i, p)
                                var mainFrameClassName = d3.select(this).attr("id").substring(8)
                                var datas = data_map.get(mainFrameClassName)
                                var classIdx = -1
                                if (p === 0) {
                                    classIdx = datas[0][3]
                                } else {
